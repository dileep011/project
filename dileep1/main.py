...
df = pd.read_csv(r"C:\Users\reddy\OneDrive\Desktop\project\Dataset\training.csv")
...
def plotPerColumnDistribution(df1: object, nGraphShown: object, nGraphPerRow: object) -> object:
    nunique = df1.nunique()
...
tr = pd.read_csv(r"C:\Users\reddy\OneDrive\Desktop\project\Dataset\testing.csv")
...
def DecisionTree():
    if len(NameEn.get()) == 0:
        ...
        self.optionmenu = tk.OptionMenu(symptom1, *OptionMenu())
        ...
        c.execute(
            "INSERT INTO DecisionTree(Name,Symtom1,Symtom2,Symtom3,Symtom4,Disease) VALUES(?,?,?,?,?,?,?)",
            (NameEn.get(), Symptom1.get(), Symptom2.get(), Symptom3.get(), Symptom4.get(), pred1.get()))
        conn.commit()
        c.close()
        conn.close()
...
def NaiveBayes():
    if len(NameEn.get()) == 0:
        ...
        c.execute("INSERT INTO NaiveBayes(Name,Symtom1,Symtom2,Symtom3,Symtom4,,Disease) VALUES(?,?,?,?,?,?,?)",
...
w2 = Label(root, justify=LEFT, text="Disease Prediction system using symptoms", fg="Black", bg="white")
...
ranfLb.grid(row=16, column=0, pady=10, sticky=W)
OPTIONS = sorted(l1)
...
lr = Button(root, text="Prediction 2", command=NaiveBayes, bg="Blue", fg="white")
...
t1 = Label(root, font=("Times", 15, "bold"), text="Decision Tree", height=1, bg="Light green"
           , width=30, fg="red", textvariable=pred1, relief="sunken").grid(row=15, column=1, padx=10)

t2 = Label(root, font=("Times", 15, "bold"), text="Naive Bayes", height=1, bg="red"
           , width=30, fg="orange", textvariable=pred2, relief="sunken").grid(row=16, column=1, padx=10)

#credits
w2 = Label(root, justify=LEFT, text="credits by dileep reddy", fg="Black", bg="white")
w2.config(font=("Times", 10, "bold"))
w2.grid(row=17, column=0, columnspan=2, padx=100)
